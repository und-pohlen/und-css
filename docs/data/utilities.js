...
module.exports = async function() {
  ...
  const pattern = /((\/\*(\=+)([\s\S]+?)\*\/))/mg
  const hashPattern = /(@[a-z]+)(\:\s)(.+)/mg

  return css.toString().match(pattern).map((item) => {
    let resp = {}
    ...
    return {
      ...
      values: resp.values.replace('(', '').replace(')', '').replace(/\"/mg, '').replace(/\s/mg, '').split(',').map((item) => {
        const i = item.split(':')
        let prop = {}
        prop[`${resp.classname}${i[0].replace(/\\+/mg, '')}`] = i[1]
        return prop
      }),
    }
  }).sort((a, b) => {
